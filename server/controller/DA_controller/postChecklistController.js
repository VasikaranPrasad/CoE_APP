const postChecklist = require("../../modal/DA_model/postChecklist");
...
// exports.createpostChecklistItem = async (req, res) => {
//   try {
...
//       const pdDevItem = await postChecklist.findOne({ userRole: "PD Dev" }).sort({ _id: -1 });
...
//       const pdLeadItem = await postChecklist.findOne({ userRole: "PD Lead" }).sort({ _id: -1 });
  ...
  try {
    ...
    if (userRole === "PD Dev") {
      ...
      if (pdDevItem) {
        ...
        // Create a new PD Dev checklist item
        checklistItem = new postChecklist({ // Use postChecklist model here
          userRole,
          data,
          pdDev: new Map(Object.entries(pdDev)),
          devComments: new Map(Object.entries(devComments)),
...
exports.getAllpostChecklistItems = async (req, res) => {
  try {
    const pdDevItem = await postChecklist.findOne({ userRole: "PD Dev" })
      .sort({ _id: -1 })
      .limit(1);

    const pdLeadItem = await postChecklist.findOne({ userRole: "PD Lead" })
      .sort({ _id: -1 })
      .limit(1);
...
exports.getPDDevpostChecklistItems = async (req, res) => {
  try {
    const pdDevItem = await postChecklist.findOne({ userRole: "PD Dev" })
      .sort({ _id: -1 })
      .limit(1);
...
exports.getPDLeadpostChecklistItems = async (req, res) => {
  try {
    const pdLeadItem = await postChecklist.findOne({ userRole: "PD Lead" })
      .sort({ _id: -1 })
      .limit(1);
