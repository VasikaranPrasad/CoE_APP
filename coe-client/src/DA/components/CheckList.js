...
function ChecklistPage() {
  ...
  useEffect(() => {
    if (userRole === "PD Lead") {
      axios
        .get(`${Backendapi.REACT_APP_BACKEND_API_URL}/api/checklist/pddev`)
        .then((response) => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(
        `${Backendapi.REACT_APP_BACKEND_API_URL}/api/checklist`,
        requestBody
      );

      // If the form submission is successful, navigate to the "preSynthesisReport" page
      navigate('/preSynthesisReport', { state: { percentage } });
